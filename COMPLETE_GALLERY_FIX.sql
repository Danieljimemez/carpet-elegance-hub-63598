-- 🎯 SETUP COMPLETO CON COLUMNAS FALTANTES
...
UPDATE gallery_photos SET set_name = COALESCE(NULLIF(regexp_replace(title, ' \d+$', ''), ''), title) WHERE set_name IS NULL;
UPDATE gallery_photos SET display_order = COALESCE(NULLIF(regexp_replace(title, '^.* ', ''), title)::INTEGER, 0) WHERE display_order IS NULL;
...
SELECT '✅ AGRUPACIÓN FUNCIONANDO' as resultado,
       set_name,
       COUNT(*) as imagenes_en_conjunto,
       STRING_AGG(title, ' → ' ORDER BY display_order) as imagenes_agrupadas
